...
bool compare_match(char *s, char *p, int n, int m)
{
	if (p[m] == '\0') return s[n] == '\0';
	bool first_match = (s[n] != '\0') && (s[n] == p[m] || p[m] == '.');
	if (p[m + 1] != '\0' && p[m + 1] == '*') {
		return (first_match && (compare_match(s, p, n + 1, m))) ||
			compare_match(s, p, n, m + 2);
	} else {
		return first_match && compare_match(s, p, n + 1, m + 1);
	}

}

bool isMatch(char *s, char *p) {
	return compare_match(s, p, 0, 0);
}
